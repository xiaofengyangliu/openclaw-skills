---
name: workplan-check
description: 工作计划过期检查提醒 - 检查飞书多维表格或本地 Excel 文件中过期未完成的工作计划，按规则分类统计并生成提醒消息。
version: 1.5.0
author: Openclaw
usage: /workplan-check [file_path [check-date] | [check-date]] [options]
keywords:
  - workplan
  - overdue-check
  - reminder
...
environment:
  - name: FEISHU_APP_ID
    description: "飞书应用 App ID，飞书多维表格模式需要"
    required: false
  - name: FEISHU_APP_SECRET
    description: "飞书应用 App Secret，飞书多维表格模式需要"
    required: false
...
options:
  - name: --sheets
    description: "Excel 模式下指定只检查哪些 sheet，多个用逗号分隔，支持模糊匹配"
  - name: --output
...
工作计划过期检查提醒技能支持飞书多维表格和本地 Excel 文件两种数据源，按照预设规则筛选计划完成时间已到但仍未完成的任务，并按责任人分组生成提醒消息。
